package com.example.studyplatform.ui.screens.explanations
...
fun ExplanationScreen() {
    ...
    if (loading) { LoadingScreen(stringResource(Res.string.explain_generating)); return }
    ...
    LazyColumn(
        ...
        item {
            AnimatedEntry {
                Column {
                    Text(
                        stringResource(Res.string.explain_title),
                        style = MaterialTheme.typography.headlineLarge,
                        color = TextPrimary
                    )
                    Spacer(Modifier.height(4.dp))
                    Text(
                        stringResource(Res.string.explain_subtitle),
        ...
        item {
            Text(stringResource(Res.string.explain_detail_level), style = MaterialTheme.typography.titleSmall, color = TextPrimary)
        ...
        item {
            PrimaryButton(text = stringResource(Res.string.explain_action), enabled = concept.isNotBlank(), onClick = {
                loading = true
                scope.launch {
                    try {
        ...
        if (history.isNotEmpty()) {
            item {
                Spacer(Modifier.height(8.dp))
                Text(stringResource(Res.string.explain_previous), style = MaterialTheme.typography.headlineSmall, color = TextPrimary)
            }
            ...
            item {
                Card(Modifier.fillMaxWidth(), colors = CardDefaults.cardColors(containerColor = WarningLight), shape = RoundedCornerShape(12.dp)) {
                    Column(Modifier.padding(16.dp)) {
                        Text(stringResource(Res.string.explain_analogy), style = MaterialTheme.typography.labelSmall, color = Warning)
                        Spacer(Modifier.height(4.dp))
        ...
        if (whenToUse != null) {
            item {
                Card(Modifier.fillMaxWidth(), colors = CardDefaults.cardColors(containerColor = SuccessLight), shape = RoundedCornerShape(12.dp)) {
                    Column(Modifier.padding(16.dp)) {
                        Text(stringResource(Res.string.explain_when_to_use), style = MaterialTheme.typography.labelSmall, color = Success)
                        Spacer(Modifier.height(4.dp))
